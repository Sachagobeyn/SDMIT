...
"Setting of working directories"
...
def read_settingsfile(settingfile):
    ...
    settings = read_file(os.path.join(cwd, settingfile), settings)
...
def overwrite_arguments(arguments):
    ...
    for i in keys:
        ...
        if "-" + i in sys.argv:
            ...
            arguments[flags[i]] = eval(sys.argv[loc + 1]) if (sys.argv[loc + 1] == "True") | (
                        sys.argv[loc + 1] == "False") else sys.argv[loc + 1]
...
def overwrite_settings(settings):
    ...
    for i in keys:
        ...
        if "-" + i in sys.argv:
            loc = sys.argv.index("-" + i)
            settings[flags[i]] = eval(sys.argv[loc + 1]) if (sys.argv[loc + 1] == "True") | (
                        sys.argv[loc + 1] == "False") else sys.argv[loc + 1]
...
def run(inputdata, taxon, variables, model_parameters, resmap, settings, full_output=False):
    ...
    inputdata, model_parameters, variables = load_and_preproces_data(inputdata, taxon, model_parameters, variables,
                                                                     resmap, settings["nan_value"])

    "Check input for the optimisation"
    check_input_code(inputdata, taxon, model_parameters, variables)

    "Optimise model"
    performance, chromosomes = optimisation(inputdata, taxon, model_parameters, variables, settings, resmap,
...
def compute_fitness(model_inputs, boundaries, chromosome, mode, nan_value, resmap, full_output, final_run=False):
    ...
    if len(variables) != 0:
        ...
        performance = {c: nan_value if (c == "AIC") or (c == "SSE") or (c == "BIC") else -nan_value for c in
                       ["AIC", "N", "K", "SSE", "AUC", "Kappa", "CCI", "Sp", "Sn", "TSS", "threshold", "BIC"]}
...
def print_models(inputdata, model_parameters, taxon, settings, chromosomes, resmap):
    ...
    for i in chromosomes:
        ...
        par_i, K = translate_chromosome(deepcopy(model_parameters[model_parameters["variable"].isin(variable)]), i,
                                        settings["mode"])
    ...
    opt_parameters, _ = translate_chromosome(
        deepcopy(model_parameters.loc[model_parameters["variable"].isin(variables)]), chromosomes[0],
        mode=settings["mode"])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        pfile = os.path.join(cwd, 'parameterfile.txt')
        print("[WARNING] No parameterfile specified, using file %s" % pfile)
        arguments = read_parameterfile(pfile)
    ...
    "Run code"
    run(arguments["inputdata"], arguments["taxon"], arguments["variables"], arguments["model_parameters"],
        arguments["resmap"], settings, full_output=eval(arguments["full_output"]))
